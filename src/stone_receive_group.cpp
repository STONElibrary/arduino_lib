...
//  int temp_value = 0;
  ...
  switch (STONER.cmd){
    ...
    case control_button_u: {
			case_btn_switch_ckbtn_rdbtn(m_user_button, 1);
    }break;
    case control_switch: {
...
void case_btn_switch_ckbtn_rdbtn (char _type, char _user){
      ...
      {
        memcpy(STONER.widget,STONE_RX_BUF+7,STONER.len-2);
        STONER.long_value = STONE_RX_BUF[7+STONER.len-2];
        STONER.long_value = STONER.long_value << 8;
        STONER.long_value += STONE_RX_BUF[7+STONER.len-1];
      }
      else
      {
...
void case_data_text (char _type){
			...
			memcpy(STONER.text,STONE_RX_BUF+8+widget_len+2,STONER.len-widget_len-3);
